 ...
 *            event => pointer to the spooler event record
 *            event_type => the Unified2 type of event
 *            arg => opaque pointer to the instance data for this plugin
 *
 *
 * Returns: void function
 *
...
void Database(Packet *p, void *event, uint32_t event_type, void *arg)
{
    ...
    if ( event == NULL || ( p == NULL && event_type != UNIFIED2_EXTRA_DATA )) 
    {
...
int dbProcessExtraData( DatabaseData *data, void *event, u_int32_t event_type )
{
    ...
    if (len) {
        extraData = SnortAlloc(len + 1); // +1 for null terminator
        memcpy(extraData, (char*) extraEvent + sizeof (Unified2ExtraData), len);
        extraData[len] = '\0';

        insert0 = (char *) SnortAlloc(len * 2 + MAX_QUERY_LENGTH + 1);
        int ret = 0;
        ...
        switch (ntohl(extraEvent->type)) {
            ...
            case EVENT_INFO_XFF_IPV6:
                ...
                packet_data_not_escaped = SnortAlloc(INET6_ADDRSTRLEN);
                inet_ntop(AF_INET6, extraData, packet_data_not_escaped, INET6_ADDRSTRLEN);
                break;

            case EVENT_INFO_XFF_IPV4:
                packet_data_not_escaped = SnortAlloc(INET6_ADDRSTRLEN);
                inet_ntop(AF_INET, extraData, packet_data_not_escaped, INET_ADDRSTRLEN);
                break;
	    ...
	    case EVENT_INFO_REVIEWED_BY:
            ...
            case EVENT_INFO_HTTP_HOSTNAME:
                ...
                packet_data_not_escaped = SnortAlloc(len+1);
                memcpy(packet_data_not_escaped,extraData,len+1);
                break;
	    ...
	    case EVENT_INFO_JSNORM_DATA:
                ...
                if (data->encoding == ENCODING_BASE64) {
                    packet_data_not_escaped = base64(extraData, len);
                } else if (data->encoding == ENCODING_ASCII) {
                    packet_data_not_escaped = ascii(extraData, len);
                } else {
                    packet_data_not_escaped = fasthex(extraData, len);
                }
                break;


            default:
                //anything we don't know about gets encoded
                DEBUG_WRAP(DebugMessage(DEBUG_LOG, "database(debug): unknown Unified2ExtraData type %d\n", ntohl(extraEvent->type)););
        }
	...
	/* This factors in the very first data->cid++ that happens on startup.  This way, the extra
        ...
        if (data->dbtype_id == DB_ORACLE) {
             ...
             * will be cut off and uploaded with OCIBindByPos().
             */
            ret = SnortSnprintf(insert0, (len * 2) + MAX_QUERY_LENGTH - 3,
                    "INSERT INTO "
...
bad_query:
    ...
    ErrorMessage("Database: Unable to construct extra query - output error or truncation\n");
    ...
    if (packet_data_not_escaped != NULL) 
    	{
	free(packet_data_not_escaped);
	}
...
int dbQuadrant(DatabaseData *data, u_int32_t sig_sid, Packet *p)
{
	...
	if ( ( SnortSnprintf(data->SQL_INSERT, MAX_QUERY_LENGTH,
		"UPDATE signature SET events_count = events_count+1 WHERE signature = %d", 
		sig_sid ) ) != SNORT_SNPRINTF_SUCCESS )
		{
		FatalError("%s() at line %u : SnortSnprintf failed for data->SQL_INSERT\n", __FUNCTION__, __LINE__);
		}
	...
	if ( ( SnortSnprintf(data->SQL_INSERT, MAX_QUERY_LENGTH,
		"INSERT INTO events_ipsrc_sig_48hr VALUES ('', %d, %d, %d, now() )",
		data->sid, data->cid, sig_sid) ) != SNORT_SNPRINTF_SUCCESS )
		{
		FatalError("%s() at line %u : SnortSnprintf failed for data->SQL_INSERT\n", __FUNCTION__, __LINE__);
		}
	...
	if ( ( SnortSnprintf(data->SQL_INSERT, MAX_QUERY_LENGTH,
		"INSERT INTO tmp_events_24 (sid, cid ,signature ,timestamp ) VALUES ( %d, %d, %d, now() )",
		data->sid, data->cid, sig_sid) ) != SNORT_SNPRINTF_SUCCESS )
		{
		FatalError("%s() at line %u : SnortSnprintf failed for data->SQL_INSERT\n", __FUNCTION__, __LINE__);
		}
        ...
        if ( ( SnortSnprintf(data->SQL_INSERT, MAX_QUERY_LENGTH,
                "INSERT INTO tmp_events_today (sid, cid ,signature ,timestamp ) VALUES ( %d, %d, %d, now() )",
                data->sid, data->cid, sig_sid) ) != SNORT_SNPRINTF_SUCCESS )
                {
                FatalError("%s() at line %u : SnortSnprintf failed for data->SQL_INSERT\n", __FUNCTION__, __LINE__);                  
                }                                                                                                                      
        ...
        if ( ( SnortSnprintf(data->SQL_INSERT, MAX_QUERY_LENGTH,
                "INSERT INTO tmp_events_yesterday (sid, cid ,signature ,timestamp ) VALUES ( %d, %d, %d, now() )",
                data->sid, data->cid, sig_sid) ) != SNORT_SNPRINTF_SUCCESS )
                {
                FatalError("%s() at line %u : SnortSnprintf failed for data->SQL_INSERT\n", __FUNCTION__, __LINE__);                  
                }                                                                                                                      
        ...
        if ( ( SnortSnprintf(data->SQL_INSERT, MAX_QUERY_LENGTH,
                "INSERT INTO tmp_events_week (sid, cid ,signature ,timestamp ) VALUES ( %d, %d, %d, now() )",
                data->sid, data->cid, sig_sid) ) != SNORT_SNPRINTF_SUCCESS )
                {
                FatalError("%s() at line %u : SnortSnprintf failed for data->SQL_INSERT\n", __FUNCTION__, __LINE__);
                }
        ...
        if ( ( SnortSnprintf(data->SQL_INSERT, MAX_QUERY_LENGTH,
                "INSERT INTO tmp_events_month (sid, cid ,signature ,timestamp ) VALUES ( %d, %d, %d, now() )",
                data->sid, data->cid, sig_sid) ) != SNORT_SNPRINTF_SUCCESS )
                {
                FatalError("%s() at line %u : SnortSnprintf failed for data->SQL_INSERT\n", __FUNCTION__, __LINE__);
                }
        ...
        if ( ( SnortSnprintf(data->SQL_INSERT, MAX_QUERY_LENGTH,
                "INSERT INTO tmp_events_quarter (sid, cid ,signature ,timestamp ) VALUES ( %d, %d, %d, now() )",
                data->sid, data->cid, sig_sid) ) != SNORT_SNPRINTF_SUCCESS )
                {
                FatalError("%s() at line %u : SnortSnprintf failed for data->SQL_INSERT\n", __FUNCTION__, __LINE__);
                }
        ...
        if ( ( SnortSnprintf(data->SQL_INSERT, MAX_QUERY_LENGTH,
                "INSERT INTO tmp_events_year (sid, cid ,signature ,timestamp ) VALUES ( %d, %d, %d, now() )",
                data->sid, data->cid, sig_sid) ) != SNORT_SNPRINTF_SUCCESS )
                {
                FatalError("%s() at line %u : SnortSnprintf failed for data->SQL_INSERT\n", __FUNCTION__, __LINE__);
                }
	...
	if ( ( SnortSnprintf(data->SQL_INSERT, MAX_QUERY_LENGTH,
		"UPDATE events_ipsrc_sig_48hr SET ip_src = %u where sid = %d and cid = %d",
		(u_long)ntohl(p->iph->ip_src.s_addr), data->sid, data->cid) ) != SNORT_SNPRINTF_SUCCESS )
		{
		FatalError("%s() at line %u : SnortSnprintf failed for data->SQL_INSERT\n", __FUNCTION__, __LINE__);
		}
	...
	if ( ( SnortSnprintf(data->SQL_INSERT, MAX_QUERY_LENGTH,
		"INSERT INTO `ip_src_unique_signatures` select '', e.ip_src, count(distinct(e.sig_id)), now() FROM events_ipsrc_sig_48hr e GROU BY ip_src") ) != SNORT_SNPRINTF_SUCCESS )
		{
		FatalError("%s() at line %u : SnortSnprintf failed for data->SQL_INSERT\n", __FUNCTION__, __LINE__);
		}
